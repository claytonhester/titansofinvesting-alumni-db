...
describe("isNameOnlyTitle", () => {
  it("flags a bare name + credentials as a bio heading", () => {
    expect(isNameOnlyTitle("Trent Aldana, CFA", "Trent Aldana")).toBe(true);
    expect(isNameOnlyTitle("Trent Aldana", "Trent Aldana")).toBe(true);
    expect(isNameOnlyTitle("Thanat Praewa, CFA®", "Thanat Praewa")).toBe(true);
  });

  it("keeps a real headline with words beyond the name", () => {
    expect(isNameOnlyTitle("Trent Aldana Named CIO of the Year", "Trent Aldana")).toBe(false);
    expect(isNameOnlyTitle("Podcast with Thanat Praewa on ETFs", "Thanat Praewa")).toBe(false);
  });
});

describe("usefulLinks", () => {
  const name = "Trent Aldana";
  ...
  it("drops firm boilerplate, filings, and name-only bio headings", () => {
    const links = [
      ...
      { label: "Trent Aldana, CFA", url: "https://warwickpartners.net/ross" },
    ];
    expect(usefulLinks(links, name)).toEqual([]);
  });
  ...
  it("keeps genuine appearances (firm bio with a real title, podcast, press)", () => {
    const links = [
      { label: "Clara Pettibone - TPH&Co.", url: "https://www.tphco.com/team/emily" },
    ...
    expect(usefulLinks(links, "Clara Pettibone").length).toBe(3);
  });
  ...
  it("drops public-records / salary-database hosts (texastaxpayers.com)", () => {
    ...
    expect(usefulLinks(links, "Bianca Rutledge")).toEqual([]);
  });
  ...
  it("drops a link whose label is a bare URL (no real title)", () => {
    ...
    expect(usefulLinks(links, "Julian Marsh")).toEqual([]);
  });
