...
def test_management_tile_not_shown():
    # Management was removed from the Overview by request; only Senior Leadership
    # remains. The data (reached_manager) still lives in person_insights.
    people = [_p(1, grad=2010, mgr=True, senior=True), _p(2, grad=2012, mgr=True)]
    keys = {s.key for s in kpi_signature_stats(people, snapshot_year=2026)}
    assert "reached_manager" not in keys
    assert "reached_senior_leadership" in keys
