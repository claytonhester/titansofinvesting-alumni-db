...
def test_undated_careers_lose_the_dated_component():
    """The Grant detector: same roles, no dates -> strictly lower score."""
...
def test_posts_link_is_not_a_profile():
    ...
    their LinkedIn profile (the Thanat case)."""
    b = compute_breakdown([
        _c("public_links", "Q&A: Sage Advisory's Thanat Praewa Speaks",
...
def test_linkedin_detected_from_linkedin_url_claim_type():
    ...
    via_value = compute_breakdown(
        [_c("linkedin_url", "https://linkedin.com/in/jlvandermeer", source_url="")]
    )
    via_url = compute_breakdown(
        [_c("linkedin_url", "search-resolved",
            source_url="https://www.linkedin.com/in/jlvandermeer")]
    )
...
def test_deep_done_person_does_not_reflag_when_still_thin():
    ...
    deep pass re-runs them at ~$0.30 forever (the Rory Fenwick case)."""
    conn = _conn_with_person()
