...
def test_subset_match_rejects_proper_subset_on_a_single_token() -> None:
    ...
    # (an M&A advisor vs an insurer). Live failure: Arvind Rao mis-attributed.
...
def test_subset_match_rejects_two_char_acronym_collision() -> None:
    ...
    # must not link unrelated firms. Live failure: Tanner Oakes (COO of Shift
...
def _seed_backfill_db(path: str) -> None:
    ...
    # The live Trent Aldana shape: a title comma-prefixed onto the firm, no ' at '.
