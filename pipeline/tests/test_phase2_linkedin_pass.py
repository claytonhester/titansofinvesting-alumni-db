...
def test_refresh_bypasses_gap_gate_for_complete_profile(monkeypatch):
    result, _ = _call(monkeypatch, policy=ResearchPolicy.REFRESH,
                      claims=list(_COMPLETE_PROFILE))
    assert result.attempted and result.claim_rows  # the Grant Larkin fix
...
# --- URL-seeded LinkedIn read (the Reid Stavros fix) --------------------------
...
def test_gated_seed_skips_read_on_rich_pdl_profile(monkeypatch):
    ...
    skips the read. This is the Reid Stavros case: PDL already deep, don't
...
def test_gated_seed_reads_corrected_url_on_thin_pdl_profile(monkeypatch):
    ...
    read still targets the corrected seed URL. The Anika/Grant case: PDL whiffed,
...
def test_resolve_seed_never_persists_for_ghost_with_zero_claims(monkeypatch):
    """The Mateo Ibarra gate finding: a 'strong' hit can be an SEO namesake echo.
