...
def test_public_salary_records_host_is_excluded_from_feed():
    ...
    (Live case: Bianca Rutledge, 'Highest Paid State Employees 2022'.)"""
    mentions = [
        _mention(
            "2022-01-01 — Highest Paid State Employees 2022",
            "Bianca Rutledge earned $408,000",
    ...
    curated, _, _ = curate_news(client, "Bianca Rutledge", "TRS", mentions)
    assert curated == []
...
def test_verify_drops_item_when_article_says_not_about():
    ...
    someone else's award entry -> verification drops it (the Trent Aldana case)."""
    ...
    curated, _, _ = curate_news(
        client, "Trent Aldana", "Warwick", mentions,
        fetch_article=lambda url: "Chris Halaska, CIO of Memorial Hermann ... would bring Trent Aldana to his team.",
    )
    assert curated == []
...
def test_verify_corrects_headline_and_summary_from_article():
    ...
    honor -> verification rewrites it (the Julian Marsh / Forbes 30 Under 30 case)."""
    mentions = [_mention("2026-01-01 — Julian Marsh - Investor, Coatue", "leads semis at Coatue")]
    ...
    client = _two_phase_client(triage, verify)
    curated, _, _ = curate_news(
        client, "Julian Marsh", "Coatue", mentions,
...
def test_verify_drops_long_article_naming_person_once():
    ...
    long_page = ("2016 Forty Under Forty. Chris Halaska, CIO of Memorial Hermann. " +
                 ...
                 "His dream team: Kris Chikelue, Trent Aldana, Danielle Villarreal.")
...
def test_news_items_excludes_social_and_directory_links():
    claims = [
        ...
        _link("Thanat — Partner", "https://theorg.com/org/sage/jane"),     # directory
...
def test_news_items_drops_name_only_bio_titles():
    ...
    claims = [
        _link("Trent Aldana, CFA", "https://www.warwickpartners.net/team/ross"),
        _link("Trent Aldana", "https://www.warwickpartners.net/ross"),
        _mention("2026-01-02 — Trent Aldana", "bio snippet"),           # name-only (dated)
        _link("Trent Aldana Named CIO of the Year", "https://www.barrons.com/x"),  # kept
    ]
    items = news_items(claims, "Trent Aldana")
    # Only the real article (extra words beyond the name) survives.
    assert len(items) == 1
    assert items[0].value == "Trent Aldana Named CIO of the Year"
...
def test_news_items_without_name_keeps_name_only_titles():
    ...
    claims = [_link("Trent Aldana, CFA", "https://www.warwickpartners.net/team/ross")]
    assert len(news_items(claims)) == 1


def test_news_items_drops_broker_echo_news_mentions():
    """Regression (Mateo Ibarra / wwana.com): a broker/SEO-echo directory page that
...
def test_curate_never_curates_broker_echo_even_if_model_would_approve():
    ...
    assert curate_news(approving, "Mateo Ibarra", "Apex", mentions) == ([], 0, 0)
...
def test_news_items_drops_boilerplate_titles():
    ...
    claims = [
        ...
        _link("Thanat on the ETF outlook", "https://www.etf.com/podcasts/x"),  # kept
    ]
    items = news_items(claims)
    assert len(items) == 1
    assert items[0].value == "Thanat on the ETF outlook"
