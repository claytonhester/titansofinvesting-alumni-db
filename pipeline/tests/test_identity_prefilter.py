...
# --- Namesake-contamination regressions (the "Dalton Ryder" failure) ----
...
def test_namesake_source_is_not_auto_accepted_for_weak_anchors() -> None:
    """A namesake SEC report (a DIFFERENT Dalton Ryder) must NOT auto-accept:
    ...
    namesake = _src(
        ...
        "Dalton Ryder, registered representative at Cetera, Cedar City, "
...
def test_city_equal_to_first_name_is_not_an_independent_anchor() -> None:
    ...
    text = _source_text(
        _src("u", "Dalton Ryder lives somewhere; University of Texas; Texas.")
    )
...
def test_real_employer_still_auto_accepts_with_same_school() -> None:
    ...
    source = _src(
        ...
        "Julian Marsh is an investor at Coatue in New York; University of Texas alum.",
    )
...
# --- Data-broker echo regression (the "Mateo Ibarra" failure) ----------------
#
...
def test_school_named_after_a_state_is_not_a_secondary_anchor() -> None:
    ...
    text = _source_text(
        _src("https://firm.com/x", "Mateo Ibarra at JP Morgan, based in Texas.")
    )
