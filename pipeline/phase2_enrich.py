...
def _resolve_linkedin_seed(
    ...
    *,
    ...
    `jean-luc-vandermeer`; the real profile is `jlvandermeer`). A plain
    ...
    # "strong" hit can be an SEO namesake echo (Mateo Ibarra + JP Morgan) that
...
def _linkedin_pass(
    ...
    *,
    ...
    Where PDL is THIN, the read is the whole résumé (Anika 0→16, Grant 0→10). So
    ...
    if not deep:
        ...
        if (not li_pass.attempted) or (not li_pass.verified_employer and li_seed_url):
            ...
            li_pass = _linkedin_pass(
                ...
                # rich PDL profile (e.g. Reid Stavros) is left to PDL and the
