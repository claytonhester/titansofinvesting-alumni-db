...
**Invariant (the Mateo rule):** no source family ever writes claims without
...
- **Zero namesake leaks** on a hand-check of 10 random enriched results
  (Grant/Anika-style side-by-side against the live LinkedIn)
...
1. **PDL quota procurement** — current cycle exhausted (API 402 despite
   ...
   or splitting across July/August resets. Deferred queue: Lin Bao,
   Zara Okonkwo, Lucia Escalante first.
...
- Mateo Ibarra–class people re-enter only via verified anchors, never bare
